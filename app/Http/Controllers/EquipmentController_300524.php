...
class EquipmentController extends Controller
{
    ...
    public function index(): View
    {
        $equipment1 = Equipment::leftjoin('equipment_relation', 'equipments.id', '=', 'equipment_relation.child_id')
            ...
            ->get()->toArray();
        $equipment2 = Equipment::leftJoin('facility_equipment_relation', 'equipments.id', '=', 'facility_equipment_relation.equipment_id')
                ->select('equipments.*','facility_equipment_relation.equipment_id',
                    'facility_equipment_relation.equipment_id' 
                )->get()->toArray();
    ...
    {
        $equipment1 = Equipment::leftjoin('equipment_relation', 'equipments.id', '=', 'equipment_relation.child_id')
        ->select('equipments.*', 'equipment_relation.parent_id', 'equipments.id')
        ->get()->toArray();
    $equipment2 = Facility::leftJoin('facility_equipment_relation', 'facilities.id', '=', 'facility_equipment_relation.facility_id')
            ->select('facilities.*', 'facilities.id')
            ->get()->toArray();
    $equipmentsOfFacilities = DB::table('equipments')
        ->join('facility_equipment_relation', 'facility_equipment_relation.equipment_id', '=', 'equipments.id')
        ->join('facilities', 'facilities.id', '=', 'facility_equipment_relation.facility_id')
        ->select('equipments.*', 'facilities.id', 'equipments.id')
        ->get()->toArray();
        
    $equipments = array_merge($equipments, $facilites, $equipmentsOfFacilities);
        return view('equipments.index', [
            'equipments' => Equipment::all(),
            'equipmentRelations' => $equipments,
        ]);
        ...
        /*public function index(): View
        {
            $equipments = Equipment::table('equipments')
                ->join('facility_equipment_relation', 'facility_equipment_relation.equipment_id', '=', 'equipments.id')
                ->join('facilities', 'facilities.id', '=', 'facility_equipment_relation.facility_id')
                ->get()->toArray();
            return view('equipments.index', [
