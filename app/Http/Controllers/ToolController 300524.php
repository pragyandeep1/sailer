...
class ToolController extends Controller
{
    ...
    public function store(StoreToolRequest $request): RedirectResponse
    {
        ...
        if ($request->faci_chkbox == 0) {
            if ($request->parent_equipment != '') {
                //save in relation table
                $equipment_relation = new EquipmentToolsRelation;
                $equipment_relation->equipment_id = $request->parent_equipment;
                $equipment_relation->tool_id = $toolId;
    ...
    public function update(UpdateToolRequest $request, Tool $tool): RedirectResponse
    {
        ...
        if ($request->faci_chkbox == 0) {
            if ($request->parent_equipment != '') {
                //save in relation table
                $equipment_relation = EquipmentToolsRelation::where('tool_id', $toolId)->first();
        ...
        $equipmentGeneralInfo = AssetGeneralInfo::where('asset_type', 'tools')->where('asset_id', $tool->id)->first();
